...
int CargarAlumno(eAlumno lista[],int tam)
{
    ...
    if( indice > -1 )
    {
        printf(" INGRESE DATOS PARA CARGAR EL ALUMNO \n");
    ...
    {
        printf("\n ERROR : No hay lugar para un nuevo alumno \n");
    }
...
/*
int MostrarAlumnos(eAlumno lista[],int tam)
{
    int i;
    for(i=0;i<tam;i++)
    {
...
{
    int indice = BuscarLibreCursos(lista,tam);
    if( indice > -1 )
    {
    ...
    {
        printf("\n ERROR : No hay lugar para un nuevo curso \n");
    }
...
void SumatoriaDeHorasDeCursosEnFuncionACantidadDeAsignados(eAlumno listaA[], int tamA, eCurso listaC[], int tamC)
{
    ...
    for(i=0;i<tamC;i++)
    {
        for(j=0;j<tamA;j++)
        {
            if(listaA[j].estado == 1 && listaA[j].idCurso == listaC[i].idCurso)
            {
                acumuladoHoras += listaC[i].horas;
            }
        }
        printf(" Por el curso %s se acumularon %d \n", listaC[i].nombre, acumuladoHoras);
        acumuladoHoras = 0;
    }
